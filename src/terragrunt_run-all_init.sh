...
function terragruntRunAllInit {
  ...
  echo "init: info: initializing Terragrunt configuration"
  ...
  if [ ${initExitCode} -eq 0 ]; then
    echo "init: info: successfully initialized Terragrunt configuration "
    echo "${initOutput}"
    echo
    exit ${initExitCode}
  fi
  ...
  echo "init: error: failed to initialize Terragrunt configuration"
  echo "${initOutput}"
  echo
